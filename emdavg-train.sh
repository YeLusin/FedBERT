...
ROOT_DIR=./emdavg/datasets_clients
CLIENTS_NUMBER=10       # Number of clients
AVG_PERIOD=50          # Period of average



for ((epoch=300;epoch<500;epoch=$epoch+$AVG_PERIOD)); do
    echo ECHO $[$epoch+1]
    for client in {0..9}; do
        ...
        fairseq-train --fp16 $ROOT_DIR/client_$client/data-bin \
        --task masked_lm --criterion masked_lm \
        --arch roberta_small --sample-break-mode complete --tokens-per-sample $TOKENS_PER_SAMPLE \
